...
-- أزرار النسخ الجديدة في أعلى القائمة
local rowGhost = makeSpamRow(3)
makeRowLabel(rowGhost, "نسخ غامض")
local copyGhostBtn = makeStartBtn(rowGhost)
local stopBtnGhost = makeStopBtn(rowGhost)

local rowPatternSpam = makeSpamRow(4)
makeRowLabel(rowPatternSpam, "نسخ سبام")
local copyPatternSpamBtn = makeStartBtn(rowPatternSpam)
local stopPatternSpamBtn = makeStopBtn(rowPatternSpam)

local rowPatternSpam2 = makeSpamRow(5)
makeRowLabel(rowPatternSpam2, "نسخ سبام 2")
local copyPatternSpam2Btn = makeStartBtn(rowPatternSpam2)
local stopPatternSpam2Btn = makeStopBtn(rowPatternSpam2)

local rowPatternSpam3 = makeSpamRow(6)
makeRowLabel(rowPatternSpam3, "نسخ سبام 3")
local copyPatternSpam3Btn = makeStartBtn(rowPatternSpam3)
local stopPatternSpam3Btn = makeStopBtn(rowPatternSpam3)

-- الأزرار الأصلية تحت الأزرار الجديدة
local rowA = makeSpamRow(7)
makeRowLabel(rowA, "سبام")
local copySpamBtn = makeStartBtn(rowA)
local stopBtnA = makeStopBtn(rowA)

local rowB = makeSpamRow(8)
makeRowLabel(rowB, "Logs")
local copyLogsBtn = makeStartBtn(rowB)
local stopBtnB = makeStopBtn(rowB)

local rowC = makeSpamRow(9)
makeRowLabel(rowC, "Re")
local copyReBtn = makeStartBtn(rowC)
local stopBtnC = makeStopBtn(rowC)

local rowD = makeSpamRow(10)
...
-- أوامر النسخ الجديدة
local function buildSequenceSpam(name, prefix, commands)
 prefix = prefix or ";"
 local out = {}
 for _, command in ipairs(commands) do
  out[#out + 1] = prefix .. command .. " " .. name
 end
 return table.concat(out, " ")
end

local function appendBlock(target, block, repeats)
 for _ = 1, repeats do
  for _, command in ipairs(block) do
   target[#target + 1] = command
  end
 end
end

local ghostBlock = {"explode", "loopkill", "clogs", "logs", "ice", "loopwarp", "freez", "jc"}
local ghostCommands = {}
appendBlock(ghostCommands, ghostBlock, 5)
local function buildCustomGhostSpam(name, prefix)
 return buildSequenceSpam(name, prefix, ghostCommands)
end

local spam1Commands = {
 "worm", "ice", "jc", "nv", "logs", "clogs",
 "worm", "ice", "jc", "nv", "logs", "clogs", "kill",
 "worm", "ice", "jc", "nv", "logs", "clogs",
 "worm", "ice", "jc", "nv", "logs", "clogs", "kill",
 "worm", "ice", "jc", "nv", "logs", "clogs",
 "worm", "ice", "jc", "nv", "logs", "clogs", "kill",
}
local function buildPatternSpam(name, prefix)
 return buildSequenceSpam(name, prefix, spam1Commands)
end

-- تم توحيد warm إلى worm لأنه نفس الأمر المستخدم في بقية التسلسل
local spam2Commands = {
 "worm", "ice", "kill", "re", "res", "logs", "clogs", "cmdbar",
 "worm", "ice", "kill", "re", "res", "logs", "clogs", "cmdbar",
 "worm", "ice", "kill", "re", "res", "logs", "clogs", "cmdbar",
 "worm", "ice", "kill", "re", "res", "logs", "clogs", "cmdbar",
 "worm", "ice", "kill", "re", "res", "logs", "clogs", "cmdbar",
...
local function buildPatternSpam2(name, prefix)
 return buildSequenceSpam(name, prefix, spam2Commands)
end

local spam3Block = {"frogify", "ice", "kill", "nv", "logs", "clogs", "cmdbar"}
local spam3Commands = {}
appendBlock(spam3Commands, spam3Block, 6)
local function buildPatternSpam3(name, prefix)
 return buildSequenceSpam(name, prefix, spam3Commands)
end
...
local patternSpamRunning = false
local patternSpam2Running = false
local patternSpam3Running = false
...
-- ── أزرار النسخ الجديدة ─────────────────────────────────────
copyPatternSpamBtn.MouseButton1Click:Connect(function()
 if patternSpamRunning then return end
 if not selectedName then setStatus("اختر لاعب اولا") return end
 local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
 startSpam(buildPatternSpam(getEffectiveName(), prefix))
 patternSpamRunning = true; setStartOn(copyPatternSpamBtn)
end)
stopPatternSpamBtn.MouseButton1Click:Connect(function()
 if patternSpamRunning then patternSpamRunning = doStop(patternSpamRunning, copyPatternSpamBtn) end
end)

copyPatternSpam2Btn.MouseButton1Click:Connect(function()
 if patternSpam2Running then return end
 if not selectedName then setStatus("اختر لاعب اولا") return end
 local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
 startSpam(buildPatternSpam2(getEffectiveName(), prefix))
 patternSpam2Running = true; setStartOn(copyPatternSpam2Btn)
end)
stopPatternSpam2Btn.MouseButton1Click:Connect(function()
 if patternSpam2Running then patternSpam2Running = doStop(patternSpam2Running, copyPatternSpam2Btn) end
end)

copyPatternSpam3Btn.MouseButton1Click:Connect(function()
 if patternSpam3Running then return end
 if not selectedName then setStatus("اختر لاعب اولا") return end
 local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
 startSpam(buildPatternSpam3(getEffectiveName(), prefix))
 patternSpam3Running = true; setStartOn(copyPatternSpam3Btn)
end)
stopPatternSpam3Btn.MouseButton1Click:Connect(function()
...
copyGhostBtn.MouseButton1Click:Connect(function()
 ...
 if not selectedName then setStatus("اختر لاعب اولا") return end
 local prefix = (prefixBox.Text ~= "" and prefixBox.Text) or ";"
 startSpam(buildCustomGhostSpam(getEffectiveName(), prefix))
...
local copySkinsBtn = makeBigBtn(ctrlScroll, "نسخ سكنات", 4,
 Color3.fromRGB(255, 120, 220), Color3.fromRGB(180, 50, 150))
